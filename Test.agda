...
strangifyp≃p p = *≃* λ @0 {(suc n) → begin
...
test-geometric-series-isConvergent = ↧ₙ (seq (proj₁ (geometric-series-isConvergent {strangify (+ 1 / 2)} lem)) 100)
  where
    open ≤-Reasoning
    -- have to use erased resp
    lem : ∣ strangify (+ 1 / 2) ∣ < 1ℝ
    lem = <-respˡ-≃0 {1ℝ} {(+ 1 / 2)⋆} {∣ strangify (+ 1 / 2) ∣} (≃-trans (≃-symm (0≤x⇒∣x∣≃x {(+ 1 / 2)⋆} (p≤q⇒p⋆≤q⋆ 0ℚᵘ (+ 1 / 2) (ℚ.*≤* (ℤ.+≤+ ℕ.z≤n)))))
                                                                        (≃-symm (∣-∣-cong (strangifyp≃p (+ 1 / 2)))))
